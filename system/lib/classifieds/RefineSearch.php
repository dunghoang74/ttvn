		...
		switch ($field['type']) {
			...
			case 'multilist':
				if ($userField == 1) {
					$query = "SELECT up.`{$fieldName}` as caption, count(`listings`.`sid`) as count
																 ...
																 {$whereStatement}
																 AND up.`{$fieldName}` != ''
																 GROUP BY `up`.`{$fieldName}` ORDER BY count DESC {$limit}";
					...
					$query = "SELECT `{$fieldName}` as caption, count(`{$fieldName}`) as count FROM `listings` {$whereStatement} AND `{$fieldName}` != '' GROUP BY `{$fieldName}` ORDER BY count DESC {$limit}";
					if (!$result = $cache->load(md5($query))) {
						$result = SJB_DB::query($query);
				...
				foreach ($result as $key => $val) {
					if (!empty($field['parent_sid'])) {
						...
						$caption = $itemInfo?$itemInfo->getValue():$val['caption'];
					}
			...
			case 'tree':
				$query = "SELECT `lt`.`sid` as `sid`, `lt`.`caption` as `value`, count(`listings`.`sid`) as `count`
																	...
																	{$whereStatement} GROUP BY `lt`.`sid` having `lt`.`sid` IS NOT NULL {$limit}";
