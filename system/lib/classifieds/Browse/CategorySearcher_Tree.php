...
class SJB_CategorySearcher_Tree extends SJB_AbstractCategorySearcher
{
	...
	public function getPositionSID($itemSID, &$result, $count, &$res)
	{
		if (isset($this->tree_values[$itemSID])) {
			if (isset($this->tree_values[$itemSID]['listing_sid'])) {
				$this->tree_values[$itemSID]['listing_sid'] += $count;
			} else {
				$this->tree_values[$itemSID]['listing_sid'] = $count;
			}
			...
			if ($this->tree_values[$itemSID]['parent_sid'] != 0 && in_array($this->tree_values[$itemSID]['parent_sid'], $this->parent)) {
				...
				self::getPositionSID($this->tree_values[$itemSID]['parent_sid'], $result, 1, $res);
			} else {
	...
	protected function _decorateItems($items)
	{
		if (SJB_Settings::getValue('enableBrowseByCounter')) {
			...
			foreach ($counts as $itemSID => $count) {
				$res = array();
				self::getPositionSID($itemSID, $result, $count, $res);
			}
		...
		if ($items) {
			...
			foreach ($items as $id => $value) {
				...
				if (SJB_Settings::getValue('enableBrowseByCounter')){
					...
					$count       = "({$this->tree_values[$id]['listing_sid']})";
					$showCounter = true;
				}
				else {
		...
		if (SJB_Settings::getValue('enableBrowseByCounter')) {
			if (is_array($itemArr)) {
				foreach ($itemArr as $item => $val ) {
					...
					$countItems = "({$this->tree_values[$item]['listing_sid']})";
