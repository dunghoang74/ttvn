...
class SJB_ListingDBManager extends SJB_ObjectDBManager
{
	...
	public static function setListingExpirationDateBySid($listing_sid)
	{
		...
		if (!empty($product_info['listing_duration']))
			SJB_DB::query('UPDATE `listings` SET `expiration_date` = NOW() + INTERVAL ?n DAY WHERE `sid` = ?n AND (`expiration_date` is NULL OR `expiration_date` < NOW())', $product_info['listing_duration'],  $listing_sid);
		return true;
	}
