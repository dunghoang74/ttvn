...
class SJB_SubAdminAcl extends SJB_Acl
{
	...
	public function getResources($type = 'all', $role = '')
	{
		...
		foreach ($listingsTypes as $listingType) {
			...
			$listingsTypesResources['manage_' . strtolower($listingType['id'])] = array(
					'title' => "Manage {$listingType['name']}",
					'group' => 'listings',
					'type' => 'subadmin'
			);
				$listingsTypesResources['get_notifications_on_' . strtolower($listingType['id']) . '_added'] = array(
					'title' => 'Get Notifications on ' . $listingType['name'] . ' Added',
					'group' => 'listings',
					'type' => 'subadmin',
					'parent' => 'manage_' . strtolower($listingType['id']),
					'notification' => true
				);
				$listingsTypesResources['get_notifications_on_' . strtolower($listingType['id']) . '_expiration'] = array(
		...
		foreach ($userGroups as $userGroup) {
			$userGroupId = strtolower($userGroup['id']);
