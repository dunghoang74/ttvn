	...
	public static function addListings($data, $usr_id, $parser_id, $script)
	{
		...
		foreach ($data as $listing) {
			...
			if (!empty($listing['ApplicationSettings'])) {
				if (preg_match("^[a-z0-9\._-]+@[a-z0-9\._-]+\.[a-z]{2,4}\$^iu",  $listing['ApplicationSettings'] )) {
			...
			if ($logo_options['option'] == 'upload_logo') {
				...
				$upload_manager->getUploadedPictureInfo($parser['xml_logo']);
				$link = $upload_manager->getUploadedFileLink($parser['xml_logo']);
