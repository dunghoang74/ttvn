...
class SJB_UserManager extends SJB_ObjectManager
{
    ...
    public static function setPasswordOriginalByUserName($username, $password_original = null)
	{
		if ($password_original)
            SJB_DB::query("UPDATE `users` SET `password_original` = ?s WHERE `username` = ?s", base64_encode(trim($password_original)), $username);
        else
            SJB_DB::query("UPDATE `users` SET `password_original` = NULL WHERE `username` = ?s", $username);
	}

    public static function getPasswordOriginalByUserName($username)
	{
        $result = SJB_DB::queryValue("SELECT `password_original` FROM `users` WHERE `username` = ?s", $username);
