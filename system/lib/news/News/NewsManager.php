...
class SJB_NewsManager extends SJB_ObjectManager
{
	...
	public static function getNewsArticleInfoBySid($sid)
 	...
 	public static function getAllNewsCountBySearchText($text, $lang = 'en', $active = false)
 	{
 		...
 		if ($active) {
 			$result = SJB_DB::query("SELECT count(*) as count FROM `news` WHERE (`brief` LIKE '%{$text}%' OR `text` LIKE '%{$text}%') AND `language` = ?s AND `active` = 1 ORDER BY `date` ASC", $lang);
 		} else {
 			$result = SJB_DB::query("SELECT count(*) as count FROM `news` WHERE (`brief` LIKE '%{$text}%' OR `text` LIKE '%{$text}%') AND `language` = ?s ORDER BY `date` ASC", $lang);
 		}
 	...
 	public static function searchArticles($text, $lang = 'en', $active = false)
 	{
 		$text   = SJB_DB::quote($text);
 		if ($active) {
 			$result = SJB_DB::query("SELECT * FROM `news` WHERE (`brief` LIKE '%{$text}%' OR `text` LIKE '%{$text}%') AND `language` = ?s AND `active` = 1 ORDER BY `date` ASC", $lang);
 		} else {
 			$result = SJB_DB::query("SELECT * FROM `news` WHERE (`brief` LIKE '%{$text}%' OR `text` LIKE '%{$text}%') AND `language` = ?s ORDER BY `date` ASC", $lang);
 		}
 		if (empty($result)) {
 			return array();
 		}
