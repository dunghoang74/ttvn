		...
		if (!is_null($user_sid)) {
			...
			if ($form_submitted && $edit_user_form->isDataValid($errors)) {
				...
				if ($form_submitted == 'save_info') {
					...
					if (in_array($userGroupInfo['id'], array('JobSeeker', 'Employer')))
						SJB_HelperFunctions::redirect(SJB_System::getSystemSettings('SITE_URL') . "/manage-".strtolower($userGroupInfo['id']).'s/');
					else
						SJB_HelperFunctions::redirect(SJB_System::getSystemSettings('SITE_URL') . "/manage-".strtolower($userGroupInfo['id']).'-users/');
				}
