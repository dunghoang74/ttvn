...
class SJB_Admin_Users_Users extends SJB_Function
{
	...
	{
		preg_match('/manage-([^.]*)\//',  SJB_Navigator::getURI(), $match);
		if ($match[1] == 'jobseekers') {
			$permissionLabel = 'jobseeker';
		}
		else if ($match[1] == 'employers') {
			$permissionLabel = 'employer';
		} else {
			$permissionLabel = preg_replace('/-users$/', '', $match[1]);
		}
		
		$this->setPermissionLabel('manage_' . $permissionLabel);
