...
class SJB_Admin_Payment_ManageInvoices extends SJB_Function
{
	...
	public function execute()
	{
				...
				case  'paid':
					foreach (array_keys($invoicesSIDs) as $invoiceSID) {
						if (SJB_PromotionsManager::isPromoCodeExpired($invoiceSID)) {
							$error = "Promotion code that was applied to this invoice is expired. Invoice cannot be paid for. Please generate a new invoice by purchasing product(s) again.";
						} else {
		...
		$tp->assign("error", isset($error) ? $error : '');
		$tp->display($template);
	}
}
