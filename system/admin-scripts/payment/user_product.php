					...
					foreach ($contracts as $key => $contractInfo) {
						...
						foreach ($listingTypes as $listingType) {
							...
							if ($acl->isAllowed('view_' . $listingTypeID . '_contact_info', $contractInfo['id'], 'contract')) {
								...
								$contractInfo['availableContactViews'][$listingTypeID]['numOfViews'] = SJB_ContractManager::getNumbeOfPagesViewed($userSID, array($contractInfo['id']), 'contact_info', $listingType['sid']);
								if (empty($permissionParam)) {
