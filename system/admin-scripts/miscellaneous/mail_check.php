...
class SJB_Admin_Miscellaneous_MailCheck extends SJB_Function
{
	...
	public function execute()
	{
		...
		if ($mailSettings['system_email']) {
			if (!preg_match('/^[a-zA-Z0-9\._-]+@[a-zA-Z0-9\._-]+\.[a-zA-Z]{2,6}$/', $mailSettings['system_email'])) {
		...
		if ($mailSettings['smtp'] == 1) {
			...
			if (!preg_match('/^[a-zA-Z0-9\._-]+@[a-zA-Z0-9\._-]+\.[a-zA-Z]{2,6}$/', $mailSettings['smtp_sender']) && !empty($mailSettings['smtp_sender'])) {
