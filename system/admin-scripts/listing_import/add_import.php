		...
		switch ($add_level) {
			...
			case '3':
				...
				if (!empty($_REQUEST['mapped']) && is_array($_REQUEST['mapped']) && !empty($original_xml) && empty($errors)) {
					...
					if ($selectedLogoOption != 'not_logo'){
					...
					$queryId = intval($id);
					$query = "SET `custom_script_users` = '{$script_users}', 
						...
						`usr_id` = '{$usr_id}', 
						`usr_name` = '{$usr_name}', 
						`maper_user` = '{$serUserMap}', 
						`xml` = '{$original_xml}', 
						`add_new_user` = '{$addNewUser}', 
						`username` = '{$username}', 
						`external_id` = '{$external_id}' 
						{$addQuery}, 
						`product_sid` = '{$selectedProduct}',
						`import_type` = '{$queryImportType}'";

					if ($id > 0) {
						SJB_DB::query("UPDATE `parsers` " . $query . " WHERE id = {$queryId}");
					} else {
						$id = SJB_DB::query('INSERT INTO `parsers` ' . $query);
					}
