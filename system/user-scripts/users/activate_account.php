				...
				if (SJB_UserManager::activateUserByUserName($_REQUEST['username'])) {
					...
					if (!SJB_Authorization::isUserLoggedIn()) {
						...
						if (!SJB_SocialPlugin::getProfileSocialID($userInfo['sid'])) {
							$password_original = SJB_UserManager::getPasswordOriginalByUserName($_REQUEST['username']);
							SJB_Notifications::sendUserWelcomeLetter($userInfo['sid'], $password_original);
							SJB_UserManager::setPasswordOriginalByUserName($_REQUEST['username']);
						}
