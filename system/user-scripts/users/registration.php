				...
				if ($isSendActivationEmail) {
					...
					if ($isSent) {
						if (!SJB_SocialPlugin::getProfileSocialID($user->getSID()))
							SJB_UserManager::setPasswordOriginalByUserName($user->getUserName(), $_REQUEST['password']['original']);
					...
					if (!SJB_SocialPlugin::getProfileSocialID($user->getSID()))
						SJB_Notifications::sendUserWelcomeLetter($user->getSID(), $_REQUEST['password']['original']);
