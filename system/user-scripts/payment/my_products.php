	...
	public function execute()
	{
		...
		foreach ($contractsInfo as $key => $contractInfo) {
			...
			foreach ($listingTypes as $listingType) {
				...
				if ($this->acl->isAllowed('view_' . $listingTypeID . '_contact_info', $contractInfo['id'], 'contract')) {
					...
					$contractInfo['avalaibleContactViews'][$listingTypeID]['numOfViews'] = SJB_ContractManager::getNumbeOfPagesViewed($currentUser->getSID(), array($contractInfo['id']), 'contact_info');
					if (empty($permissionParam)) {
		...
		foreach ($listingTypes as $listingType) {
			...
			if ($this->acl->isAllowed('view_' . $listingTypeID . '_contact_info')) {
				...
				$statistics['avalaibleContactViews'][$listingTypeID]['numOfViews'] = SJB_ContractManager::getNumbeOfPagesViewed($currentUser->getSID(), $contractSIDs, 'contact_info');
				if (empty($permissionParam)) {
