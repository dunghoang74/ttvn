...
{if $socialAutoFillData.allow}
	<div id="social_autoFill" class="{$socialAutoFillData.network}_16">
		{if $socialAutoFillData.logged}
			...
			{else}
				<a href="{$GLOBALS.site_url}/add-listing/?listing_type_id={$listingTypeID|htmlentities}&amp;product_sid={$product_sid}&amp;autofill" title="">[[Auto-fill resume from my {$socialAutoFillData.network} profile]]</a>
			{/if}
		{else}
...
<form method="post" action="{$GLOBALS.site_url}/add-listing/{$listingTypeID|htmlentities}/{$currentPage.page_id}/{$listingSID}" enctype="multipart/form-data" {if $form_fields.ApplicationSettings}onsubmit="return validateForm('addListingForm');"{/if} id="addListingForm" class="inputForm">
	...
	{$picValue = (!empty($listing_sid)) ? "{$GLOBALS.site_url}/manage-pictures/?listing_sid={$listing_sid}" : "{$GLOBALS.site_url}/manage-pictures/?{if $contract_id != 0}contract_id={$contract_id}&amp;{/if}product_sid={$product_sid}&amp;listing_sid={$listing_id}"}
