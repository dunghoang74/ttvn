{if !isset($listing_duration)}
	{$listing_duration = (isset($contract['listing_duration'])) ? $contract['listing_duration'] : $productInfo['listing_duration']}
{/if}

{if $listing_duration}
	{if $listing['activation_date']}
		{$maxExpirationDate = strftime("{$GLOBALS.current_language_data.date_format}", strtotime("+{$listing_duration} day", strtotime($listing['activation_date'])))}
	{else}
		{$maxExpirationDate = strftime("{$GLOBALS.current_language_data.date_format}", strtotime("+{$listing_duration} day"))}
	{/if}
{/if}
...
<table id="expiration-date">
	<tr>
		...
		<td>
			<label for="maximum-length">[[Maximum length]] ({if $listing_duration}{$listing_duration} [[days]]{else}[[Unlimited]]{/if})</label>
...
<script type="text/javascript">
	...
	var id = '{$id}';
	var listingDuration = '{$listing_duration}';
	var expired = '{$expired}';
	if (expired) {
