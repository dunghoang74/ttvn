<input type="text" value="{$value.value|escape:'html'}" class="inputStringMoney {if $complexField}complexField{/if}" name="{if $complexField}{$complexField}[{$id}][{$complexStep}][value]{else}{$id}[value]{/if}" />
