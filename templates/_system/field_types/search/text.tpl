...
{else}
	<input type="text" value="{if $id == 'keywords'}{$value.all_words|escape:'html'}{$value.exact_phrase|escape:'html'}{$value.any_words|escape:'html'}{$value.boolean|escape:'html'}{else}{$value.like|escape:'html'}{/if}" class="searchText" name="{$id}[{if $id == 'keywords'}all_words{else}like{/if}]"  id="{$id}" />
{/if}
