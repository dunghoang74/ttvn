...
<fieldset>
	...
	<form method="post" enctype="multipart/form-data">
		<div>
			<table>
				<tr><td width="150px">Upload Banner File</td><td><input type="radio" name="bannerType" value="file" {if $params.bannerType == 'file' || $params.bannerType == ''}checked="checked"{/if} onclick="chooseBannerType('file');" /></td></tr>
				<tr><td>Insert Banner Code</td><td><input type="radio" name="bannerType" value="code"  {if $params.bannerType == 'code'}checked="checked"{/if} onclick="chooseBannerType('code');"/></td></tr>
			</table>	
		</div>
		<table>
			...
			{foreach from=$banner_fields item=form_field}
				<tr  id="{$form_field.id}" {if (($params.bannerType == 'file' || $params.bannerType == '' || !$params.bannerType) && $form_field.id == 'code') || ($params.bannerType == 'code' && $form_field.id == 'image')}style="display:none;"{/if}>
					<td valign="top" width="200">{$form_field.caption}</td>
					<td valign="top" width="1">{if $form_field.id == 'link' || $form_field.id == 'title' || $form_field.id == 'image' || $form_field.id == 'code'}<span style="color:red;">*</span>{/if}</td>
					<td valign="middle" style="font-size: 10px;" width="1"></td>
					<td>	
