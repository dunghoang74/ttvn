...
<form method="post" action="{$GLOBALS.site_url}/clone-product/" id="productForm">
	...
	<div id="addProduct">
		...
		{/foreach}
		<div style="width: 920px;"><input type="submit" class="grayButton" value="Save" id="saveProduct"  /></div>
	</div>
</form>
</div>
...
<script type="text/javascript">
    ...
    function changePermissions(user_group_sid)
    {
        if (user_group_sid != 0) {
			...
			$.post(url, {'user_group_sid': user_group_sid, 'product_type': '{/literal}{$product_type}{literal}', 'permissions_type': 'additional', 'role': '{/literal}{$product_info.sid}{literal}', 'params': '{/literal}{$params}{literal}'},  function(data){
				$("#additional_permissions").html(data);
			});
			if (access_settins == 1) {
				$.post(url, {'user_group_sid': user_group_sid, 'product_type': '{/literal}{$product_type}{literal}', 'permissions_type': 'access', 'role': '{/literal}{$product_info.sid}{literal}', 'params': '{/literal}{$params}{literal}'},  function(data){
