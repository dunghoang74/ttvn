...
<head>
    ...
    <title>[[Admin Panel]] {if $TITLE ne ""} :: [[{$TITLE}]] {/if}</title>
	...
	{* Users *}
		...
		{elseif $url === '/system/users/acl/' && $params}
			http://wiki.smartjobboard.com/display/sjb42/User+Groups
		{elseif $url === '/manage-jobseekers/' || $url === '/add-jobseeker/' || $GLOBALS.wikiExtraParam == 'JobSeeker'}
			http://wiki.smartjobboard.com/display/sjb42/Manage+Job+Seekers
		{elseif $url === '/manage-employers/' || $url === '/add-employer/' || $GLOBALS.wikiExtraParam == 'Employer'}
