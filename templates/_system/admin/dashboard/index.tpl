...
<div id="stats">
	<div id="statblocks">
		...
		{foreach key=key name=outer item=groupInfo from=$groupsInfo}
			<div class="statblock">
				<h2>
					{if in_array($key, array('JobSeeker', 'Employer'))}
						<a href="{$GLOBALS.site_url}/manage-{$key|lower}s/?user_group%5Bequal%5D={$key}&amp;action=search">{$groupInfo.total.count}</a>
					{else}
						<a href="{$GLOBALS.site_url}/manage-{$key|lower}-users/?user_group%5Bequal%5D={$key}&amp;action=search">{$groupInfo.total.count}</a>
					{/if}
				</h2>
		...
		{foreach key=key name=outer item=listingInfo from=$listingsInfo}
			{if $smarty.foreach.outer.first}
				{if $totalFlagsNum.$key > 0}
					<div class="statblock">
						<h2><a href="{$GLOBALS.site_url}/flagged-listings/?listing_type_id={$key}">
								<strong style="color: #C00;">{$totalFlagsNum.$key}</strong></a></h2>
				...
				{if $onlineUsers}
					{foreach key=key name=outer item=value from=$onlineUsers}
						<tr class="{cycle values = 'evenrow,oddrow' advance=true}">
							...
							<td align="center">
								{if in_array($key, array('JobSeeker', 'Employer'))}
									<a href="{$GLOBALS.site_url}/manage-{$key|lower}s/?user_group%5Bequal%5D={$key}&amp;online=1&amp;action=search">{$value.count} online</a>
								{else}
									<a href="{$GLOBALS.site_url}/manage-{$key|lower}-users/?user_group%5Bequal%5D={$key}&amp;online=1&amp;action=search">{$value.count} online</a>
								{/if}
							</td>
						</tr>
					{/foreach}
...
<div class="dashboardBlocks">
	<div class="box">
		...
		<div class="innerpadding">
			{foreach key=key name=outer item=groupInfo from=$groupsInfo}
				{if in_array($key, array('JobSeeker', 'Employer'))}
					{capture name="users_url"}manage-{$key|lower}s{/capture}
				{else}
					{capture name="users_url"}manage-{$key|lower}-users{/capture}
				{/if}
				{assign var="users_url" value=$smarty.capture.users_url}
