			...
			<div id="file_{if $complexField}{$complexField}[{$id}][{$complexStep}]{else}{$id}{/if}" style="float:left;">
				<div class="complex-view-file-caption">
					{if $filesInfo.$complexStep.saved_file_name}
						<a href="?listing_id={$listing.id}&amp;filename={$filesInfo.$complexStep.saved_file_name|escape:'url'}&amp;complex_field={$complexField}:{$id}:{$complexStep}">{$filesInfo.$complexStep.file_name|escape:'html'}</a>
					{else}
