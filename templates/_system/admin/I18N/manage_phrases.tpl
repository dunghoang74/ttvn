...
<script type="text/javascript">
	...
	$(function() {
		var content = '<img src="{/literal}{$GLOBALS.user_site_url}/system/ext/jquery/progbar.gif{literal}" />';
		
		$(".goEditPhrase").click(function(){
			$("#editPhraseDialog").dialog('destroy');
			$("#editPhraseDialog").attr({title: "{/literal}{$smarty.capture.loading|escape:"javascript"}{literal}"});
			$("#editPhraseDialog").html(content).dialog({width: 200});
			var link = $(this).attr("href");
			$.get(link, function(data){
				$("#editPhraseDialog").dialog('destroy');
				$("#editPhraseDialog").attr({title: "{/literal}{$smarty.capture.edit_phrase|escape:"javascript"}{literal}"});
