{literal}
    ...
    $( function() {
        ...
        if (action == 'new') {
            $('#save').attr('value', '{/literal}[[Add]]{literal}');
        }
    });
    ...
    function moveTo ( link, caption ) {
        ...
        $("#dialog").attr({title: "{/literal}[[Loading]]{literal}"});
        $("#dialog").html(progbar).dialog({width: 180});
        $.get(link, function(data){
            $("#dialog").dialog('destroy');
            $("#dialog").attr({title: "{/literal}[[Move]]{literal} " + caption});
            ...
            <tbody>
                {foreach from=$fieldsOnPage item=fieldOnPage name=fieldList}
                    <tr class="{cycle values = 'evenrow,oddrow' advance=true}">
                        <td class="dragHandle" {if $fieldOnPage.type == 'location'} style="padding: 0px" {/if}>
                        ...
                        <td {if $fieldOnPage.type == 'location'} style="padding: 0px" {/if}>
                        	{if $fieldOnPage.type == 'location'}
