...
<table>
	...
	<tbody>
		...
		<tr class="{if $counter is odd}oddrow{else}evenrow{/if}">
			...
			<td><a href="{$GLOBALS.site_url}/listing-feeds/?action=delete&amp;feedId={$feed.sid}" onclick="return confirm('Are you sure you want to delete the \'{$feed.name}\' item?')" title="Delete" class="deletebutton">Delete</a></a></td>
		</tr>
	</tbody>
{/foreach}
</table>