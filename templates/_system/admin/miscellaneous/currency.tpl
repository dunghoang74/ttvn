		...
		<tbody>
			{foreach from=$currency item=curr key=currency_id}
				<tr class="{cycle values="oddrow,evenrow"}">
					...
					<td>{if $curr.active == 1}<a href='?action=deactivate&sid={$curr.sid}' class="deletebutton">Deactivate<a/>{else}<a href='?action=activate&sid={$curr.sid}' class="editbutton greenbtn" style="width: 60px; text-align: center;">Activate<a/>{/if}</td>
...
{elseif $action eq 'add'}
	...
	<h1><img src="{image}/icons/recycle32.png" border="0" alt="" class="titleicon"/>{if $button == 'Edit'}Edit{else}Add a New{/if} Currency</h1>
