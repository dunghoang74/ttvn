...
$(function() {
    ...
    $('#parser_user_username').blur( function() {
			...
			<tr id="clearTable">
				<td colspan='2'>
					...
					<div style='font-size:11px; font-style:italic;'>
						...
						$listing - is a current listing being imported. E.g. $listing['Location_Country'] - contains Country value of imported listing.<br />
						$skip = true; - use this script to skip listing meeting a certain criteria. E.g. to skip all listings from Texas use<br />
						if ($listing['Location_State'] == 'Texas')<br />
    					&nbsp;&nbsp;$skip = true;
					</div>
				</td>
			</tr>
			<tr id="clearTable">
				<td colspan='2'>
					...
					<div style='font-size:11px; font-style:italic;'>
						You can enter script in PHP here that will be executed for each import iteration.<br />
						You can use the following variables:<br />
						$user - is a current user being imported. E.g. $user['Location_Country'] - contains Country value of imported user.
					</div>
				</td>
			</tr>
			<tr id="clearTable">
...
function set_logoOptions(){
    ...
    var link = '{/literal}{$logo_link}{literal}';
...
function deleteLogo()
{
    $.get('{/literal}{$GLOBALS.site_url}/edit-import/?id={$id}&logo_id={$xml_logo}{literal}', function(){
...
function getProducts()
{
    ...
    var url = '{/literal}{$GLOBALS.site_url}/edit-import/{literal}';
    if (parser_user) {
        $.ajax({
           url: url,
           type: 'POST',
           ...
           success: function(data){
			   ...
			   var defaultValue = '{/literal}<option value="">{$smarty.capture.selectProduct|escape:"javascript"}</option>{literal}';
