...
{breadcrumbs}<a href="{$GLOBALS.site_url}/manage-{if $user_group_info.id == 'Employer' || $user_group_info.id == 'JobSeeker'}{$user_group_info.id|lower}s{else}{$user_group_info.id|lower}-users{/if}/{if $restore}?restore=1{/if}">Manage {if $user_group_info.id == 'Employer' || $user_group_info.id == 'JobSeeker'}{$user_group_info.name}s{else}'{$user_group_info.name}' Users{/if}</a> &#187; Edit {$user_group_info.name}{/breadcrumbs}
	...
	<form method="post" enctype="multipart/form-data" id="editUserForm">
		...
		<table>
			...
			<tr>
				<td valign="top">IP</td>
				<td valign="top"></td>
				<td>{if $user_info.ip_is_banned}<a href="{$GLOBALS.site_url}/banned-ips/" target="_blank" style="color: red!important">{$user_info.ip}</a>{else}{$user_info.ip}{/if}</td>
			</tr>
			<tr>
				<td colspan="3">
