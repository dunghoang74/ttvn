...
<div id="choose_user_search_criteria">
	...
	<div class="setting_block" style="display: none"  id="clearTable">
		<form method="get" name="search_form" onSubmit="return formSubmit('{$GLOBALS.site_url}/manage-{if $userGroupInfo.id == 'Employer' || $userGroupInfo.id == 'JobSeeker'}{$userGroupInfo.id|lower}s{else}{$userGroupInfo.id|lower}-users{/if}/', 'searchForm')" id="searchForm">
				...
				<tr>
					...
					<td><select name="product[simple_equal]">
						...
						{foreach from=$products item=product}
							<option value="{$product.sid}" {if $selectedProduct.simple_equal eq $product.sid}selected="selected"{/if}>{$product.name}</option>
						{/foreach}
						</select>
					</td>
				</tr>
