...
{breadcrumbs}
    {if $type == 'user'}
    	<a href="{$GLOBALS.site_url}/manage-{if $userGroupInfo.id == 'Employer' || $userGroupInfo.id == 'JobSeeker'}{$userGroupInfo.id|lower}s{else}{$userGroupInfo.id|lower}-users{/if}/">Manage {if $userGroupInfo.id == 'Employer' || $userGroupInfo.id == 'JobSeeker'}{$userGroupInfo.name}s{else}'{$userGroupInfo.name}' Users{/if}</a> &#187; <a href="{$GLOBALS.site_url}/edit-user/?user_sid={$role}">Edit {$userGroupInfo.name}</a> &#187; View Permissions
    {elseif $type == 'group'}
