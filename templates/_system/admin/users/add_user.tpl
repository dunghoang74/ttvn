{breadcrumbs}<a href="{$GLOBALS.site_url}/manage-{if $user_group.id == 'Employer' || $user_group.id == 'JobSeeker'}{$user_group.id|lower}s{else}{$user_group.id|lower}-users{/if}/">Manage {if $user_group.id == 'Employer' || $user_group.id == 'JobSeeker'}{$user_group.name}s{else}'{$user_group.name}' Users{/if}</a> &#187; Add a New {$user_group.name}{/breadcrumbs}
...
<fieldset>
	...
	<form id="editUserForm" method="post" enctype="multipart/form-data" action="{$GLOBALS.site_url}/add-{$user_group.id|lower}/" onsubmit="disableSubmitButton('submitAdd');">
		{set_token_field}
