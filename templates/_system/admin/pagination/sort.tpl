		...
		{if $fieldInfo.isVisible == true}
			<th>
				{if $fieldInfo.isSort == false}
					[[{$fieldInfo.name}]]
				{else}
					<a href="?{if $paginationInfo.restore == 1}restore=1&amp;{/if}sortingField={$fieldKey}&amp;sortingOrder={if $paginationInfo.sortingOrder == 'ASC' && $paginationInfo.sortingField == $fieldKey}DESC{else}ASC{/if}&amp;itemsPerPage={$paginationInfo.itemsPerPage}&amp;page=1{if $paginationInfo.uniqueUrlParams}{foreach from=$paginationInfo.uniqueUrlParams key=id item=param}&amp;{$id}={if $param.escape}{$param.value|escape:"{$param.escape}"}{else}{$param.value}{/if}{/foreach}{/if}">[[{$fieldInfo.name}]]</a>
